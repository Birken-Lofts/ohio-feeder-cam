...
- **Camera/Restream**: `CAM_USER`, `CAM_PASS`, `CAM_IP`, `CAM_PATH`, `RESTREAM_URL`, `RESTREAM_KEY`. Note: in current Restream, *every* encoder/RTMP stream key contains `_event` (`re_<id>_event<hash>`) — that's just how Restream names the encoder ingest key, and per Restream's docs it is **persistent** (stays the same unless you refresh it). So a `_event` key is fine to reuse; there is no separate "non-event" key to hunt for. If ffmpeg gets `Error opening output ... End of file` and the pipeline restart-loops every ~10s, the key is valid but the **stream on Restream's side is stopped/ended/scheduled-for-later** — open (or re-create) the encoder stream in the dashboard and leave it running; ffmpeg connecting is what brings it live.
- **Encode**: `OVERLAY_FPS`, `X264_PRESET`, `VBITRATE`, `VBUFSIZE`, `TZ` (drives the on-screen clock). Keep `VBITRATE` ≤ **4500k** for 1080p30 — that's Restream's recommended Full-HD ceiling; pushing well above it (e.g. 8000k) makes Restream's ingest buffer/jitter and the stream stutters even when the encoder itself is keeping up. `VBUFSIZE` ≈ 2× `VBITRATE`.
...
- Diagnosing a **choppy/stuttering output**: it's usually *not* the encoder. On the dev Mac, libx264 1080p30 `veryfast` benchmarks at ~16× real-time and the live encoder uses ~1 core, so CPU is rarely the constraint. Two real causes, in order: (1) **over-bitrate to Restream** — see the `VBITRATE` note above (4500k ceiling); (2) **host contention** — the dev Mac runs a heavy software transcode at only ~1.0× margin, so anything else stealing cores (other VMs, *and the native counter, which pulls a second RTSP client + runs YOLO*) can cause micro-stalls. To isolate: run `ffmpeg ... -f null -` with `-stats` and watch `speed=`/`drop=`; a stable copy (e.g. camera→YouTube `-c copy`) being smooth while our re-encode stutters points at bitrate/contention, not the code. The real home for the transcode is the prod NVENC box, which has copy-like headroom.
